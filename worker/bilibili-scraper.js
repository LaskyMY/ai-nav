...
function generateVideoPage(meta) {
  ...
  // Parse intro for structured content
...
.card:hover{transform:translateY(-2px);border-color:rgba(99,102,241,.2);box-shadow:0 0 80px rgba(99,102,241,.1),0 8px 32px rgba(0,0,0,.2)}
.card:active{transform:scale(.98);transition:transform .12s ease}

/* ── Typography (spec §十三) ── */
h2{font-size:17px;font-weight:700;color:var(--cyan);margin:0 0 10px;padding-left:12px;border-left:3px solid var(--cyan)}
h3{font-size:15px;font-weight:600;color:rgba(255,255,255,.85);margin:14px 0 6px}
...
/* ── Content Text ── */
.section-body{font-size:14px;color:var(--tx2);line-height:1.8}
...
.key-points li{padding:6px 0;font-size:14px;color:var(--tx2);line-height:1.7;border-bottom:1px solid rgba(255,255,255,.03);display:flex;gap:8px}
.key-points li .kp-dot{width:6px;height:6px;border-radius:50%;background:var(--cyan);flex-shrink:0;margin-top:7px}
...
/* ── Canvas Animation (subtle, follows spec §六: only transform+opacity) ── */
...
@media(max-width:500px){.wrap{padding:14px}h1{font-size:22px}.meta-grid{grid-template-columns:repeat(2,1fr)}.section-body{font-size:13px}.reading-mode{gap:4px}}
</style>
</head>
<body>
...
<div class="wrap">
<!-- Hero Cover (with referrerpolicy for B站 CDN) -->
...
<!-- ── Section 1: 一分钟速览 ── -->
<div class="content-section card" id="s1">
  <h2>📋 视频信息</h2>
  <div class="meta-grid">
    ...
    <div class="meta-item"><span class="meta-icon">💬</span><b>${danmuStr}</b><span style="font-size:10px;color:var(--tx3)">弹幕</span></div>
...
<!-- ── Section 2: 三分钟内容梳理 ── -->
<div class="content-section card" id="s2">
  <h2>📝 内容梳理</h2>
...
<!-- ── Section 3: 六分钟深读 ── -->
<div class="content-section card" id="s3">
  ${intro ? `<h2>📖 完整简介</h2><div class="desc">${escapeHtml(intro) || "(暂无简介)"}</div>` : ''}
</div>
...
// Canvas particle animation (GPU-only: transform+opacity per spec §六)
(function(){
  var c=document.getElementById('animC'),ctx=c.getContext('2d');
  var W,H,particles=[];
  function resize(){W=c.parentElement.clientWidth;H=c.parentElement.clientHeight;c.width=W;c.height=H}
  resize();window.addEventListener('resize',resize);
  for(var i=0;i<35;i++){particles.push({x:Math.random()*W,y:Math.random()*H,r:Math.random()*2+1,vx:(Math.random()-.5)*.4,vy:(Math.random()-.5)*.4,a:Math.random()*.25+.08})}
  function anim(){
    ctx.clearRect(0,0,W,H);
    for(var i=0;i<particles.length;i++){var p=particles[i];p.x+=p.vx;p.y+=p.vy;if(p.x<0)p.x=W;if(p.x>W)p.x=0;if(p.y<0)p.y=H;if(p.y>H)p.y=0;ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fillStyle='rgba(99,102,241,'+p.a+')';ctx.fill()}
    for(var i=0;i<particles.length;i++){for(var j=i+1;j<particles.length;j++){var dx=particles[i].x-particles[j].x,dy=particles[i].y-particles[j].y;if(dx*dx+dy*dy<6400){ctx.beginPath();ctx.moveTo(particles[i].x,particles[i].y);ctx.lineTo(particles[j].x,particles[j].y);ctx.strokeStyle='rgba(99,102,241,'+(.05*(1-Math.sqrt(dx*dx+dy*dy)/80))+')';ctx.lineWidth=.5;ctx.stroke()}}}
    requestAnimationFrame(anim);
  }
  anim();
})();

// Glow (spec §4.3: rAF lerp .05)
